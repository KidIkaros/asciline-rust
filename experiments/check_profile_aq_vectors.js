#!/usr/bin/env node
/**
 * check_profile_aq_vectors.js — decode the Rust tag-5 (adaptive quantization)
 ...
 * the per-block AQ map + scaled quant tables cross the wire identically.
 *
...
const FILE = path.join(__dirname, 'vectors_profile_aq_rust.bin');
...
(async () => {
  ...
  while (off + HEADER <= buf.length) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (msg.length < 5 || msg[4] !== codec.TAG_PROFILE_AQ) {
        throw new Error(`frame ${index} (${w}x${h}): not a tag-5 message (tag=${msg.length >= 5 ? msg[4] : 'short'})`);
      }
  ...
  console.log(`OK: codec.js decoded ${frames} Rust AQ-encoded frames across ${cases} cases, all bit-exact`);
})().catch((e) => {
