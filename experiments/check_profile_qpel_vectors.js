#!/usr/bin/env node
/**
 * check_profile_qpel_vectors.js — decode the Rust tag-7 (quarter-pixel motion)
 ...
 * the H.264 6-tap half-pel filter + bilinear quarter-pel interpolation
 * (identical integer math in Rust and web/codec.js) crosses the wire
...
const FILE = path.join(__dirname, 'vectors_profile_qpel_rust.bin');
...
(async () => {
  ...
  while (off + HEADER <= buf.length) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (msg.length < 5 || msg[4] !== codec.TAG_PROFILE_QPEL) {
        throw new Error(`frame ${index} (${w}x${h}): not a tag-7 message (tag=${msg.length >= 5 ? msg[4] : 'short'})`);
      }
  ...
  console.log(`OK: codec.js decoded ${frames} Rust quarter-pel-encoded frames across ${cases} cases, all bit-exact`);
})().catch((e) => {
