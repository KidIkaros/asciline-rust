...
# AQ levels (--aq) and half-pixel motion (--hpel) against a pinned source,
# reporting output size + PSNR-Y/SSIM-Y + wall time so the rate-distortion
# trade-off is measurable. RDO uses SATD distortion (Hadamard) + a
# coefficient-count rate, so the lambda scale differs from an SSE-based cost.
#
# Usage:
...
run_case() {
    local label="$1" r="$2" lam="$3" aq="$4" hpel="$5"
    local out="$TMP/ab" t0 t1 size psnr ssim wall
    local hflag=""; [ "$hpel" = 1 ] && hflag="--hpel"
    t0=$(date +%s.%N)
    "$BIN" "$SOURCE" --cols "$COLS" --profile --qf "$QF" --fps "$FPS" \
        --r-search "$r" --rdo-lambda "$lam" --aq "$aq" $hflag --out "$out" > "$TMP/report.txt" 2>&1
    t1=$(date +%s.%N)
...
run_case "r7 hpel"         7   0     2   1
run_case "r7 hpel aq0"     7   0     0   1
